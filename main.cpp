...
bool getRulesList(std::string dir, 
	std::vector<std::string> & files,std::string suff);
...
int main(int argc, char** argv)
{
	...
	g_signal_connect_swapped (G_OBJECT (quitButton), "clicked",
		      ...
		      G_OBJECT (window));
	g_signal_connect_swapped (G_OBJECT(startButton),"clicked",
					G_CALLBACK (getLoopy), 
					G_OBJECT(ruleAdjust));
	g_signal_connect_swapped (G_OBJECT(stopButton),"clicked",
					G_CALLBACK (stopLoopy), 
					G_OBJECT(ruleAdjust));
	g_signal_connect_swapped (G_OBJECT(resetButton),"clicked",
					G_CALLBACK (resetImage), 
					G_OBJECT(resetButton));
	g_signal_connect_swapped(G_OBJECT(ruleAdjust),"value-changed",
					G_CALLBACK (updateRule), 
					GTK_WIDGET(ruleAdjust));
	g_signal_connect_swapped(G_OBJECT(widthAdjust),"value-changed",
					G_CALLBACK (initCA), 
					GTK_WIDGET(widthAdjust));
					
	g_signal_connect_swapped(G_OBJECT(saveButton),"clicked",
					G_CALLBACK (getPic), 
					GTK_WIDGET(saveButton));
	g_signal_connect_swapped(G_OBJECT(checkButton),"toggled",
					G_CALLBACK (toggleSave), 
	...
	for(uint i=0;i<rulesList.size();i++)
	{
		gtk_combo_box_append_text(
			GTK_COMBO_BOX(rulesComboBox),rulesList[i].c_str());
	}
	if(rulesList.size())
	{
	...
	g_signal_connect(G_OBJECT(rulesComboBox),
		"changed",G_CALLBACK(combo_selected),rulesFrame);
	g_signal_connect_swapped (G_OBJECT(rulesClearButton),"clicked",
				G_CALLBACK (clearRules), 
				G_OBJECT(rulesClearButton));
					
	autoResetButton = gtk_check_button_new_with_mnemonic(
		"_Auto Reset on Dead Lines");
	randomizeLengthButton = gtk_check_button_new_with_mnemonic(
		"Randomize _Time Between Rule Changes");
	
	g_signal_connect_swapped (G_OBJECT(autoResetButton),"toggled",
				G_CALLBACK (autoResetFunction), 
				G_OBJECT(autoResetButton));
	g_signal_connect_swapped (G_OBJECT(randomizeLengthButton),"toggled",
				G_CALLBACK (randomizeLengthFunction), 
				G_OBJECT(randomizeLengthButton));
	...
	gtk_box_pack_start(GTK_BOX(rulesVBox),
		randomizeLengthButton,FALSE,FALSE,0);
...
{
	ca1->setRule(
		gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(adjustor)));
}
...
gint loopThrough(gpointer data)
{
	...
	if(rules.size()>0)
	{
		if((numLines%ruleChangeInterval==0)&&(numLines!=0))
		{
			...
			std::cout << "changing rule to " << 
				rules[seed] << std::endl;
			if(gtk_toggle_button_get_active(
				GTK_TOGGLE_BUTTON(randomizeLengthButton)))
			{
				ruleChangeInterval=rand()%
					maxRuleChangeInterval+1;
			}
		}
	}
...
void initCA()
{
	...
	bool seed = FALSE;
	unsigned int width = gtk_spin_button_get_value_as_int(
		GTK_SPIN_BUTTON(widthAdjust));
	...
	ca1->resetCells();
	//initialize the rule
	ca1->setRule(gtk_spin_button_get_value_as_int(
		GTK_SPIN_BUTTON(ruleAdjust)));
}
